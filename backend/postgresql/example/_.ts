...
export class ExamplePostgresql {

    static sqlDb = postgres(`postgres://${userName}:${password}@${hostAddress}:${portNumber}/${dbName}`);


    static async createIndex(columns: string[]) {
        const sql = this._createIndexSqlString(columns)
        await ExamplePostgresql.sqlDb.unsafe(sql).execute()
    }
    ...
    static _createIndexSqlString(columns: string[]): string {
        ...
        const tableName = "ExampleModel"; // Replace with your actual table name
        return `CREATE INDEX ${indexName} ON ${tableName} (${columns.join(", ")});`;
    }


    static async createTable() {
        const createTableSQL: string = `CREATE TABLE IF NOT EXISTS Example(` +
        ...
        await ExamplePostgresql.sqlDb.unsafe(createTableSQL).execute();
    }
    ...
    static async insert(object: Example) {
        ...
        const sql = `insert into Example (` +
            ...
            `,'${Base64.encode(JSON.stringify(object.j000.map((model: Sub) => model.toDataString())))}'` +
    ...
    static async update(object: Example) {
        const sql = `update Example set ` +
            ...
            //
            `j000 = '${Base64.encode(JSON.stringify(object.j000.map((model: Sub) => model.toDataString())))}',` +
    ...
    static async delete(docId: string) {
        const sql = `delete from Example where docId = '${docId}'`;
        await ExamplePostgresql.sqlDb.unsafe(sql).execute();
    }
    ...
    static async get(docId: string): Promise<Example | null> {
        ...
        const sql = `select * from Example where DocId = '${docId}'`;

        console.log('sql: ', sql)

        let xs = await ExamplePostgresql.sqlDb.unsafe(sql).execute();

        if (xs.length == 0) {
            return null;
        }
        // console.log('xs[0][docid]: ', xs[0]['DocId'.toLowerCase()])
        return ExamplePostgresql.fromMap(xs[0]);
    }
    ...
    static async getAll(): Promise<Example[]> {
        ...
        for (let i = 0; i < xs.length; i++) {
            result.push(ExamplePostgresql.fromMap(xs[i]));
        }
    ...
    static async getByI000(value: number): Promise<Example | null> {
        const sql = `// select * from Example where i000 = ${value}`;
        let xs = await ExamplePostgresql.sqlDb.unsafe(sql).execute();
    ...
    static fromMap(queryParams: any): Example {
        ...
        object.c000 = Sub.fromDataString(Base64.decode(queryParams.c000) || new Sub().toDataString());
        ...
        //
        object.j000 = (JSON.parse(queryParams.j000 ? Base64.decode(queryParams.j000) : '[]') || []).map((item: string) => Sub.fromDataString(item));
        ...
        //
        object.e000 = TestEnumTestHelper.fromString(queryParams.e000 ? Base64.decode(queryParams.e000) : TestEnumTest.notSelected);
