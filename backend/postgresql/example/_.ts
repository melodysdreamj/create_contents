...
export class CheckPostgresql {
  ...
  // static async getByI000(value: number): Promise<Check | null> {
  //   const sql = 'SELECT "docId" FROM "Check" WHERE "i000" = ${value}'; // 컬럼명, 테이블명 확인
  //   const params = { value };
  //   const result = await CheckPostgresql.db.oneOrNone(sql, params);
  //   return result ? CheckPostgresql.fromMap(result) : null;
  // }
  ...
  static async resetTable() {
    ...
    await CheckPostgresql.db.none(sql);
    // console.log("Table Check dropped (if existed).");
