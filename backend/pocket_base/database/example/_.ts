...
export class ExamplePocketBaseCollection {
  ...
  static async insert(object: Example): Promise<string> {
    await ExamplePocketBaseCollection.getDb();
    ...
    const recordData = {
      ...
      j000: JSON.stringify(object.j000.map((model: Sub) => model.toDataString())),
      e000: object.e000,
      // s000: object.s000,
      // i000: object.i000,
      // b000: object.b000 ? 1 : 0,
  ...
  static async upsert(object: Example): Promise<string | null> {
    await ExamplePocketBaseCollection.getDb();
  ...
  static async upsertMany(objects: Example[]): Promise<string[] | null> {
    if (objects.length === 0) {
      return [];
    }
        ...
        for (const object of chunk) {
          ...
          try {
            if (existingRecord) {
              ...
              // 생성
              const result = await pb.collection("Example").create(recordData);
              allResultIds.push(result.id);
            }
          } catch (e) {
            console.error(
  ...
  static async getAll(): Promise<Example[]> {
    ...
    try {
      while (true) {
        ...
        const resultList = await pb.collection("Example").getList(page, perPage);
      ...
      return allRecords.map((record: any) => Example.fromMap(record));
    } catch (error) {
      console.error(
        `[Example] Error during manual pagination on page ${page}:`,
        error
      );
  ...
  static async getByI000(value: number): Promise<Example | null> {
    ...
    try {
      const xs = await pb.collection("Example").getFirstListItem(`i000=${value}`);
  ...
  static async upsertFile(object: Example, filePath: string) {
    ...
    try {
      if (obj == null) {
        ...
        const uploadResponse: AxiosResponse<any> = await axios.patch(
          `${pb.baseUrl}/api/collections/Example/records/${id}`,
          formData,
          {
            headers: {
              ...formData.getHeaders(),
        ...
        // Axios를 사용하여 업로드 요청 수행
        const uploadResponse: AxiosResponse<any> = await axios.patch(
          `${pb.baseUrl}/api/collections/Example/records/${obj.id}`,
          formData,
          {
            headers: {
              ...formData.getHeaders(),
  ...
  static async upsertFileFromStream(object: Example, stream: Readable) {
    await ExamplePocketBaseCollection.getDb();
    ...
    try {
      ...
      // Axios를 사용하여 업로드 요청 수행
      const uploadResponse: AxiosResponse<any> = await axios.patch(
        `${pb.baseUrl}/api/collections/Example/records/${recordId}`,
        formData,
        {
          headers: {
            ...formData.getHeaders(),
  ...
  static async upsertFileFromBuffer(object: Example, buffer: Buffer) {
    await ExamplePocketBaseCollection.getDb();
  ...
  static async downloadFile(object: Example, filePath: string): Promise<boolean> {
    await ExamplePocketBaseCollection.getDb();
  ...
  static async getDownloadUrl(object: Example | null): Promise<string | null> {
    if (object == null) {
  ...
  static async downloadFileAsBuffer(object: Example): Promise<Buffer | null> {
    await ExamplePocketBaseCollection.getDb();
