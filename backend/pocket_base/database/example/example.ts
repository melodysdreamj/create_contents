...
export class ExamplePocketBaseCollection {
    ...
    static async upsert(object: Example) {
        ...
        if (rawObj == null) {
            ...
            try {
                const updatedRecord = await pb.collection('Example').update(rawObj.id, updatedData);
            } catch (e) {
    ...
    static async getAll(): Promise<Example[]> {
        ...
        try {
            ...
            while (hasMore) {
                // 각 페이지별 레코드 리스트 가져오기
                const resultList = await pb.collection('Example').getList({page: page, perPage: perPage});
    ...
    static async resetTable() {
        ...
        // 이름으로 컬렉션 찾기
        const collectionToDelete = collections.find((collection: any) => collection.name === 'Example');
