...
export class ShareGptData {
    ...
    toDataString(): string {
        return btoa(Array.from(new TextEncoder().encode(new URLSearchParams({
            ...
            talks: JSON.stringify(this.talks.map((model: Talk) => model.toDataString())),
    ...
    static fromDataString(dataString: string): ShareGptData {
        ...
        object.talks = (JSON.parse(queryParams["talks"] || "[]") || []).map((item: string) => Talk.fromDataString(item));
...
export class ShareGptDataSqlite {
    ...
    static async getDb(): Promise<sqlite3.Database> {
        if (!this.dbInstance) {
            ...
            // Create the ShareGptData table if it doesn't exist
    ...
    private static async createTable() {
        const createTableSQL: string = `CREATE TABLE IF NOT EXISTS ShareGptData(` +
    ...
    static async insert(object: ShareGptData) {
        ...
        const sql = `insert into ShareGptData (` +
            `docId` +
            // `,s000` +
            // `,s001` +
            // `,s002` +
    ...
    static async update(object: ShareGptData) {
        ...
        const sql = `update ShareGptData set ` +
            `docId = ?` +
            // `,s000 = ?` +
            // `,s001 = ?` +
            // `,s002 = ?` +
