...
export class ExampleRedisGeo {
    ...
    static async getDb() {
        if (ExampleRedisGeo._ready) return;
        dotenv.config();
        // 로그인 (아이디와 비밀번호 설정 필요)
        ExampleRedisGeo.ref = new Redis({
            url: `https://${process.env.UPSTASH_URL}`,
            token: process.env.UPSTASH_TOKEN,
        });

        ExampleRedisGeo._ready = true;
    }
    ...
    static async upsert(key: string, longitude: number, latitude: number, member: string): Promise<void> {
        await ExampleRedisGeo.getDb();
        await ExampleRedisGeo.ref.geoadd(`geo:Example:${key}`, { longitude, latitude, member });
    }
...
// 가까운 위치 데이터 100개 가져오기 (반경을 포함하여 검색)
    static async getNearby(key: string, longitude: number, latitude: number, radius: number,count: number): Promise<any[] | null> {
        try {
            await ExampleRedisGeo.getDb();
            const result = await ExampleRedisGeo.ref.geosearch(
                `geo:Example:${key}`,
                {
                    type: "FROMLONLAT",
    ...
    static async deleteMember(key: string, member: string): Promise<void> {
        await ExampleRedisGeo.getDb();
        await ExampleRedisGeo.ref.zrem(`geo:Example:${key}`, member); // GEO 데이터는 Sorted Set 내부에 저장됨
    }
    ...
    static async getDistance(key: string, member1: string, member2: string): Promise<number | null> {
        try {
            await ExampleRedisGeo.getDb();
            const result = await ExampleRedisGeo.ref.geodist(`geo:Example:${key}`, member1, member2, 'KM');
    ...
    static async getCoordinates(key: string, member: string): Promise<any[] | null> {
        try {
            await ExampleRedisGeo.getDb();
            const result = await ExampleRedisGeo.ref.geopos(`geo:Example:${key}`, member);
    ...
    static async getGeoHash(key: string, member: string): Promise<string | null> {
        try {
            await ExampleRedisGeo.getDb();
            const result = await ExampleRedisGeo.ref.geohash(`geo:Example:${key}`, member);
