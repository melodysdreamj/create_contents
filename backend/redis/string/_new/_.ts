...
export class NewRedisString {
    ...
    private static async getDb(): Promise<RedisClientType> {
        ...
        if (!NewRedisString.isDotenvLoaded) {
            dotenv.config(); // .env 파일에서 환경 변수 로드
            NewRedisString.isDotenvLoaded = true;
        }

        // 이미 연결이 되어 있으면 해당 Redis 인스턴스를 반환
        if (NewRedisString.ref) return NewRedisString.ref;
        ...
        // Redis 인스턴스를 저장
        NewRedisString.ref = client;
        console.log(`Connected to Redis at ${redisHost}:${redisPort}`);

        return NewRedisString.ref;
    }

    // 데이터를 삽입하거나 업데이트하는 메서드 (UPSERT)
    static async upsert(key: string, value: string): Promise<void> {
        const redis = await NewRedisString.getDb();  // Redis 연결 인스턴스 가져오기
        await redis.set(`str:New:${key}`, value);
    }
    ...
    static async get(key: string): Promise<string | null> {
        try {
            const redis = await NewRedisString.getDb();  // Redis 연결 인스턴스 가져오기
            const result = await redis.get(`str:New:${key}`);
    ...
    static async delete(key: string): Promise<void> {
        const redis = await NewRedisString.getDb();  // Redis 연결 인스턴스 가져오기
        await redis.del(`str:New:${key}`);
    }

    // 문자열에 값을 추가하는 함수 (기존 문자열에 append)
    static async append(key: string, value: string): Promise<number | null> {
        try {
            const redis = await NewRedisString.getDb();
            const result = await redis.append(`str:New:${key}`, value);
    ...
    static async strLen(key: string): Promise<number | null> {
        try {
            const redis = await NewRedisString.getDb();
            const result = await redis.strLen(`str:New:${key}`);
    ...
    static async getRange(key: string, start: number, end: number): Promise<string | null> {
        try {
            const redis = await NewRedisString.getDb();
            const result = await redis.getRange(`str:New:${key}`, start, end);
    ...
    static async setRange(key: string, offset: number, value: string): Promise<number | null> {
        try {
            const redis = await NewRedisString.getDb();
            const result = await redis.setRange(`str:New:${key}`, offset, value);
