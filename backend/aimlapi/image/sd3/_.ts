...
export async function generateSd3ImageUrl(prompt: string): Promise<string | null> {
    dotenv.config();
...
export async function downloadAndSaveSd3Image(imageUrl: string, savePath: string): Promise<string | null> {
    try {
...
export async function generateSd3ImageSaveImage(prompt: string): Promise<string | null> {
    dotenv.config();
    ...
    if (imageUrl) {
        ...
        // 이미지 다운로드 및 저장
        return await downloadAndSaveSd3Image(imageUrl, savePath);
    }
    return null;
}